...
#    RespondImg = RespondImg/RespondImgNorm_
    ...
    # RespondImg = np.array(RespondImg)
    GridMask = cp.array(GridCounter>0, dtype=cp.int32)
    # GridMask = np.array(GridCounter>0, dtype=np.int32)
    ...
    # NeighborFeatureDiffs = np.zeros((RespondImg.shape[0], RespondImg.shape[1], nWindowArea, RespondImg.shape[2]), dtype=np.float64)
    ...
    # NeighborMaskMap = np.zeros((RespondImg.shape[0], RespondImg.shape[1], nWindowArea), dtype=np.int32)
    # get diffs map
...
#     KeyPixels = np.array(aAllDiffs[aAllDiffs.shape[0]-nFinalKeyPts:aAllDiffs.shape[0], 4:6], dtype=np.int32)
    ...
    print("KeyPixels:", KeyPixels)
    print("PlanarPts: ", PlanarPts)
    print("KeyPts.shape[0]:",  KeyPts.shape[0])
